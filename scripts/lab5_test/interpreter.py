    ...
    def _init_instructions(self):
        ...
        def _parse_line(line):
            if re.match(InstructionEnum.Mov.value, line):
                ...
                args = _parse_args(line, 2)
                return ImulInstruction(InstructionEnum.Imul, args[0], args[1])
            ...
            elif re.match(InstructionEnum.Set.value, line):
                space_index = line.find(' ')
                condition = SetEnum.E
                for name, member in SetEnum.__members__.items():
                    if line[:space_index] == member.value:
                        condition = member
                return SetInstruction(InstructionEnum.Set, condition, line[space_index:].strip())
