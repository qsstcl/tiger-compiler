...
build() {
  build_target=$1
  cd "$WORKDIR" && mkdir -p build && cd build && cmake -DCMAKE_BUILD_TYPE=Debug .. >/dev/null && make "$build_target" -j >/dev/null
  if [[ $? != 0 ]]; then
...
test_lab5_part1() {
  ...
  for testcase in "$testcase_dir"/*.tig; do
    ...
    if [[ $? != 0 ]]; then
      ...
      echo "Error log can be found at ${testcase}.err.log"
      full_score=0
      continue
    fi
    ...
    if [[ $? != 0 ]]; then
      echo -e "\e[31mError\e[0m: Link error [$testcase_name]"
      echo "Check your llvm file at ${testcase}.ll"
      full_score=0
      continue
    fi
